...
func TestStaticPolicyAddWithUncoreAlignment(t *testing.T) {
	...
	testCases := []staticPolicyTestWithResvList{
		...
		{
			// Compatibility with Topology Hints
			description:     "GuPodSingleContainer, DualSocketSMTUncore, FullNUMAsAvail, ExpectNUMANodeAffinity, ExpectAllocOneUncore",
			topo:            topoDualSocketSingleNumaPerSocketSMTUncore,
			numReservedCPUs: 8,
			reserved:        cpuset.New(0, 1, 96, 97, 192, 193, 288, 289), // note 4 cpus taken from uncore 0, 4 from uncore 12
			cpuPolicyOptions: map[string]string{
			...
			topologyHint: &topologymanager.TopologyHint{NUMANodeAffinity: newNUMAAffinity(1), Preferred: true}, //hint on NUMANode 1
			//expected CPU alignment to UncoreCacheID 13 (first fully available Uncore Cache on NUMANode 1)
			expUncoreCache: cpuset.New(13),
		},
		{
		...
		{
			// Compatibility with Topology Hints
			description:     "GuPodMultiContainer, DualSocketSMTUncore, FullNUMAsAvail, ExpectNUMANodeAffinity, FragmentedUncore, ExpectAllocThreeUncore",
			topo:            topoSmallDualSocketSingleNumaPerSocketSMTUncore, // 16 cpus per uncore
			numReservedCPUs: 8,
			reserved:        cpuset.New(0, 1, 32, 33, 64, 65, 96, 97), // note 4 cpus taken from uncore 0, 4 from uncore 4
			cpuPolicyOptions: map[string]string{
				FullPCPUsOnlyOption:            "true",
				PreferAlignByUnCoreCacheOption: "true",
			},
			stAssignments: state.ContainerCPUAssignments{},
			// uncore 5 fully allocated
			stDefaultCPUSet: topoSmallDualSocketSingleNumaPerSocketSMTUncore.CPUDetails.CPUs().Difference(
				topoSmallDualSocketSingleNumaPerSocketSMTUncore.CPUDetails.CPUsInUncoreCaches(5),
			),
			pod: WithPodUID(
				makeMultiContainerPod(
					[]struct{ request, limit string }{}, // init container
					[]struct{ request, limit string }{ // app container
						{"16000m", "16000m"}, // full uncore cache worth of cpus
						{"16000m", "16000m"}, // full uncore cache worth of cpus
						{"8000m", "8000m"},
					},
				),
			...
			topologyHint:   &topologymanager.TopologyHint{NUMANodeAffinity: newNUMAAffinity(1), Preferred: true}, //hint on NUMANode 1
			expUncoreCache: cpuset.New(4, 6, 7),
		},
		{
		...
		{
			// Compatibility with Topology Hints
			description:     "GuPodSingleContainer, DualSocketNoSMTUncore, FullNUMAsAvail, ExpectNUMANodeAffinity, MultiNUMA, ExpectAllocTwoUncore",
			...
			pod: WithPodUID(
				makeMultiContainerPod(
					[]struct{ request, limit string }{}, // init container
					[]struct{ request, limit string }{ // app container
						{"12000m", "12000m"}, // larger than an uncore
					},
				),
				"with-single-container",
			),
			topologyHint:   &topologymanager.TopologyHint{NUMANodeAffinity: newNUMAAffinity(3), Preferred: true}, //hint on NUMANode 3
			expUncoreCache: cpuset.New(18, 19),
		},
		{
		...
		{
			// Compatibility with Topology Hints on monolithic x86 architectures
			description:     "GuPodSingleContainer, MonoUncoreCacheHT, SubNUMACluster, ExpectAllocCPUSet",
			topo:            topoDualSocketSubNumaPerSocketHTMonolithicUncore, // Uncore cache CPUs = Socket CPUs
			numReservedCPUs: 4,
			reserved:        cpuset.New(0, 1, 120, 121), // note 4 cpus taken from first 2 cores
			cpuPolicyOptions: map[string]string{
			...
			topologyHint: &topologymanager.TopologyHint{NUMANodeAffinity: newNUMAAffinity(3), Preferred: true}, //hint on NUMANode 3
			expCPUAlloc:  true,
			expCSet:      cpuset.New(90, 91, 210, 211),
		},
		{
		...
		},
		{
			...
			description:     "GuPodSingleContainer, DualSocketUncore, ExpectNUMANodeAffinity, PartialUncore, ExpectAllocTwoUncore",
			topo:            topoSmallDualSocketSingleNumaPerSocketNoSMTUncore, // 8 cpus per uncore
			numReservedCPUs: 4,
			reserved:        cpuset.New(0, 1, 2, 3), // note 4 cpus taken from uncore 0
			cpuPolicyOptions: map[string]string{
			...
			topologyHint:   &topologymanager.TopologyHint{NUMANodeAffinity: newNUMAAffinity(0), Preferred: true}, //hint on NUMANode 0
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			// if the test case specifies expected UncoreCacheIDs, check if there was a proper allocation
			// otherwise check if a single Uncore Cache was allocated
			if !testCase.expUncoreCache.IsEmpty() {
